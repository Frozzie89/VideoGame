...
void State_Game::MouseClick(EventDetails *l_details)
{
    ...
    if (mousePos.x >= m_btnEndTurn.getPosition().x && mousePos.x <= m_btnEndTurn.getPosition().x && mousePos.y >= m_btnEndTurn.getPosition().y && mousePos.y <= m_btnEndTurn.getPosition().y)
    {
        m_fight.endTurn();
